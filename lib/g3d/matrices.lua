...
function matrix:lookAtFrom(pos, target, up)
    self[4]  = pos[1]
    self[8]  = pos[2]
    self[12] = pos[3]

    -- resetting axes resets scale
    local sx, sy, sz = self:getScale()

    -- forward direction
    local z1, z2, z3 = vectorNormalize(pos[1]-target[1], pos[2]-target[2], pos[3]-target[3])
    --side direction
    ...
    self[1], self[2], self[3]   = z1, x1, y1
    self[5], self[6], self[7]   = z2, x2, y2 
    self[9], self[10], self[11] = z3, x3, y3 
